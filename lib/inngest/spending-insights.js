...
export const spendingInsights = inngest.createFunction(
  ...
  { cron: "0 8 1 * *" }, // Runs on the 1st of every month at 08:00 UTC
  async ({ step }) => {
    ...
    // Step 2: Generate insights and send emails for each user
    for (const user of users) {
      // a. Fetch user’s monthly expenses
      const expenses = await step.run(`Expenses · ${user._id}`, () =>
        convex.query(api.inngest.getUserMonthlyExpenses, { userId: user._id })
      );
      if (!expenses?.length) continue;

      // b. Prepare expense summary for AI prompt
      ...
      // c. Build AI prompt for spending analysis
      const prompt = `
...
Use a friendly, encouraging tone. Do not wrap your response in any markdown code blocks or use backticks. Just return pure HTML only.
...
5. Recommendations for Next Month
      ...
      try {
        // Generate insights using Gemini AI (retry-aware via step.ai.wrap)
        ...
        // d. Send personalized spending insights email
    ...
    // Step 3: Return summary for logs
    return {
