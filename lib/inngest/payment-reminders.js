...
export const paymentReminders = inngest.createFunction(
  ...
  { cron: "0 10 * * *" }, // Scheduled daily at 10 AM UTC
  async ({ step }) => {
    // Step 1: Fetch all users with outstanding debts
    const users = await step.run("fetch-debts", () =>
      convex.query(api.inngest.getUsersWithOutstandingDebts)
    );

    // Step 2: Build and send reminder emails for each user
    const results = await step.run("send-emails", async () => {
      return Promise.all(
        users.map(async (u) => {
          // Build table rows for the user's debts
          const rows = u.debts
            .map(
              (d) => `
                <tr>
          ...
          // Skip if user has no outstanding debts
          if (!rows) return { userId: u._id, skipped: true };

          // Construct reminder email content
          const html = `
          ...
          // Attempt to send the email via Convex action
          try {
    ...
    // Step 3: Return summary of the operation
    return {
